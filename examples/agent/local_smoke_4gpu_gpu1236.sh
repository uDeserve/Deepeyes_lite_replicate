...
# 4卡冒烟配置（方案C：使用GPU 1,2,3,6 避开GPU 0）
# 策略：选择当前空闲的GPU组合，避免GPU 0被同门进程占用
#
# Usage:
# 1) source ~/deepeyes_raid_env.sh
# 2) export PATH="/data/lsj/deepeyes/conda_env/bin:$PATH"
# 3) export CUDA_VISIBLE_DEVICES=1,2,3,6
...
# 8) bash examples/agent/local_smoke_4gpu_gpu1236.sh

PROJECT_NAME="agent_vlagent_local"
EXPERIMENT_NAME="smoke_4gpu_gpu1236"
