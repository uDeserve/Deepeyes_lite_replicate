...
# 2×GPU 冒烟（推荐物理卡 5、6，避开 0–4 上他人任务）。
# 若日志出现 GPU 上多个 PID 分显存仍 OOM，请先 nvidia-smi 确认 5、6 无他人占卡。
#
# Usage:
# 1) source ~/deepeyes_raid_env.sh
# 2) conda activate /data/lsj/deepeyes/conda_env
# 3) export CUDA_VISIBLE_DEVICES=5,6
# 4) export WORLD_SIZE=1
# 5) export LLM_AS_A_JUDGE_BASE="https://your-endpoint/v1"
# 6) export LLM_AS_A_JUDGE_API_KEY="your-api-key"
# 7) export LLM_AS_A_JUDGE_MODEL="qwen3.5-plus"
# 8) bash examples/agent/local_smoke_2gpu_gpu56.sh

PROJECT_NAME="agent_vlagent_local"
EXPERIMENT_NAME="smoke_2gpu_gpu56_subset512"
